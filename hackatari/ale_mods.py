...
class ALEInpainting:
    ...
    def custom_getScreenRGB(self, *args, **kwargs):
        ret = self._ale.getScreenRGB(*args, **kwargs)
        for inpaintings in self._inpaintings:
            for x, y, w, h, subimage in inpaintings():
    ...
    def place_above(self, colors):
        ...
        for c in colors:
            col.extend(c())
        assert all(len(c) == 3 for c in col), "Colors need to be RGB tuples"
        self._place_above = col
        if colors:
