'''
hacked_fishing_derby.py implements an altered version of FishingDerby in which
the behavior of the shark and fish can be changed in various ways. 
...
- 3 = The x-position of the fish is changed. They are in the middle betwen 
      player and enemy.
And the following modes for the shark:
...
# Parser argument for changing the shark behavior.
parser.add_argument("-sm", "--sharkmode", default=0, type=int,
                     help="Argument for changing the shark mode. The value has to be between 0-4."
...
class HackedFishingDerby(OCAtari):
    '''
    HackedFishingDerby: Modifies the Atari game "FishingDerby" such that the shark and fish can be manipulated.
    '''
...
class HackedFishingDerbyHuman(OCAtari):
    '''
    HackedFishingDerbyHuman: Enables human play mode for the HackedFishingDerby game.
    '''
    ...
    def __init__(self, env_name: str):
        '''
        Initializes the HackedFishingDerbyHuman environment with the specified environment 
        name.
        '''
    ...
    def run(self):
        '''
        run: Runs the HackedFishingDerbyHuman environment, allowing human interaction
        with the game.
        '''
        self.running = True
        while self.running:
    ...
    def _handle_user_input(self):
        '''
        _handle_user_input: Handles user input for the HackedFishingDerbyHuman environment.
        '''
...
if TOGGLE_HUMAN_MODE:
    renderer = HackedFishingDerbyHuman('FishingDerby')
    renderer.run()
else:
    env = HackedFishingDerby(render_mode="human")
